...
type ChapterScope='Future'|'Past'|'All'
type MilestoneStrength='Exceptional'|'Very Strong'|'Strong'|'Notable'
type MilestoneIndicator={label:string;detail:string;weight:number}
type MajorChapter={
  id:string
  area:string
  icon:string
...
const MILESTONE_AREAS:MilestoneArea[]=[
  {key:'identity',name:'Identity & personal direction',icon:'✦',houses:[1],angle:'ASC',manifestations:['personal reinvention','new independence','a major change of direction','a defining personal commitment']},
  {key:'resources',name:'Money & material foundations',icon:'◇',houses:[2,8],manifestations:['income or compensation shift','major purchase or investment','shared-resource decision','financial restructuring']},
  {key:'home',name:'Home, family & place',icon:'⌂',houses:[4],angle:'IC',manifestations:['relocation','property or housing change','family-role shift','a new long-term base']},
  {key:'creative',name:'Love, creativity & legacy',icon:'✺',houses:[5],manifestations:['important romance','creative launch','children or mentoring responsibilities','a passion project becoming serious']},
  {key:'relationship',name:'Partnership & commitment',icon:'∞',houses:[7],angle:'DSC',manifestations:['major partnership','marriage or formal commitment','a decisive relationship redefinition','a consequential alliance']},
  {key:'world',name:'Travel, study & worldview',icon:'⌁',houses:[9],manifestations:['long-distance move or travel','higher study or certification','publishing or teaching','immigration or cross-border developments']},
  {key:'career',name:'Career & public direction',icon:'△',houses:[10],angle:'MC',manifestations:['promotion or leadership','new profession or business','public recognition','a major professional pivot']},
  {key:'community',name:'Networks, audience & alliances',icon:'◎',houses:[11],manifestations:['a powerful new network','community leadership','audience growth','an important collaboration or patron']},
]

function closestMajorAspect(a:number,b:number,maxOrb=3.2){
  const sep=angularDistance(a,b)
  let best:{name:string;angle:number;orb:number}|null=null
  for(const asp of ASPECTS){
    const delta=Math.abs(sep-asp.angle)
    if(delta<=maxOrb && (!best||delta<best.orb)) best={name:asp.name,angle:asp.angle,orb:delta}
  }
  return best
}

function chapterAngle(chart:ChartData,angle:MilestoneArea['angle']){
  if(angle==='ASC')return chart.ascendant
  if(angle==='DSC')return norm(chart.ascendant+180)
  if(angle==='MC')return chart.mc
  if(angle==='IC')return norm(chart.mc+180)
  return null
}
...
function milestoneStrength(score:number):MilestoneStrength{
  if(score>=6.6)return 'Exceptional'
  if(score>=5.5)return 'Very Strong'
  if(score>=4.5)return 'Strong'
  return 'Notable'
}

function dedupeIndicators(items:MilestoneIndicator[]){
  const seen=new Set<string>()
  return [...items].sort((a,b)=>b.weight-a.weight).filter(x=>{
    const k=`${x.label}|${x.detail}`
    if(seen.has(k))return false
    seen.add(k);return true
  })
}

function majorLifeChapters(swe:SwissEphemeris,chart:ChartData,birthDate:string):MajorChapter[]{
  const [birthYear,birthMonth,birthDay]=birthDate.split('-').map(Number)
  const now=new Date()
  const currentAge=ageOnDate(birthDate,now)
  const startAge=Math.max(12,currentAge-12)
  const endAge=Math.min(75,currentAge+24)
  const natalJupiter=chart.planets.find(p=>p.name==='Jupiter')
  const natalSaturn=chart.planets.find(p=>p.name==='Saturn')
  const candidates:MajorChapter[]=[]

  for(let age=startAge;age<=endAge;age++){
    const year=birthYear+age
    const birthday=new Date(Date.UTC(year,birthMonth-1,birthDay,12))
    const profectedAge=ageOnDate(birthDate,birthday)
    const profHouse=(profectedAge%12)+1
    const profSign=(chart.ascSignIndex+profHouse-1)%12
    const lord=SIGNS[profSign][2]
    const natalLord=chart.planets.find(p=>p.name===lord)

    // Sample the twelve months of the profection year from birthday to birthday,
    // rather than using a calendar year (which would apply the wrong profection before the birthday).
    const monthDates=Array.from({length:12},(_,offset)=>{
      const d=new Date(Date.UTC(year,birthMonth-1+offset,birthDay,12))
    ...
    for(const area of MILESTONE_AREAS){
      let annual=0
      const annualEvidence:MilestoneIndicator[]=[]
      if(area.houses.includes(profHouse)){
        annual+=2.55
        annualEvidence.push({label:'Annual profection',detail:`Age ${profectedAge} activates House ${profHouse} (${HOUSE_NAMES[profHouse-1]}), placing this life area at the center of the year.`,weight:2.55})
        if([1,4,7,10].includes(profHouse)){
          annual+=.55
          annualEvidence.push({label:'Angular profection',detail:`House ${profHouse} is angular, traditionally making the year more outwardly eventful and visible.`,weight:.55})
        }
      }
      if(natalLord && area.houses.includes(natalLord.house)){
        annual+=.42
        annualEvidence.push({label:'Lord of the Year',detail:`${lord}, ruler of the profected year, is natally placed in House ${natalLord.house}, tying the annual time-lord to this sphere.`,weight:.42})
      }
      if(natalLord && [1,4,7,10].includes(natalLord.house) && area.houses.includes(profHouse)){
        annual+=.38
        annualEvidence.push({label:'Angular time-lord',detail:`Natal ${lord} is angular in House ${natalLord.house}, strengthening the year's capacity to show up concretely.`,weight:.38})
      }

      const rulers=[...new Set(area.houses.map(h=>SIGNS[(chart.ascSignIndex+h-1)%12][2]))]
      const targetNatal=rulers.map(r=>chart.planets.find(p=>p.name===r)).filter((p):p is PlanetRow=>!!p)
      const angleLon=chapterAngle(chart,area.angle)
      const monthScores:{score:number;items:MilestoneIndicator[]}[]=months.map(transits=>{
        let score=0
        const items:MilestoneIndicator[]=[]
        for(const t of transits){
          const houseWeight=t.name==='Jupiter'?.88:t.name==='Saturn'?.95:t.name==='Mars'?.34:t.name==='Venus'?.30:t.name==='Mercury'?.14:t.name==='Sun'?.12:.05
          if(area.houses.includes(t.house)){
            score+=houseWeight
            if(t.name==='Jupiter'||t.name==='Saturn')items.push({label:`${t.name} through the topic house`,detail:`${t.name} moves through House ${t.house}, keeping ${area.name.toLowerCase()} under sustained activation.`,weight:houseWeight})
          }

          if(['Jupiter','Saturn','Mars','Venus'].includes(t.name)){
            for(const target of targetNatal){
              const hit=closestMajorAspect(t.longitude,target.longitude,3.2)
              if(hit){
                const base=t.name==='Saturn'?1.08:t.name==='Jupiter'?1.0:t.name==='Mars'?.42:.36
                const w=base*Math.max(.18,1-hit.orb/3.2)
                score+=w
                if(w>.28)items.push({label:'House-ruler contact',detail:`Transiting ${t.name} forms a ${hit.name.toLowerCase()} to natal ${target.name}, ruler of a house in this life area (${hit.orb.toFixed(1)}° orb).`,weight:w})
              }
            }
          }

          if(angleLon!==null && (t.name==='Jupiter'||t.name==='Saturn')){
            const hit=closestMajorAspect(t.longitude,angleLon,3.2)
            if(hit){
              const w=(t.name==='Saturn'?1.22:1.12)*Math.max(.2,1-hit.orb/3.2)
              score+=w
              items.push({label:`${area.angle} activation`,detail:`${t.name} forms a ${hit.name.toLowerCase()} to the natal ${area.angle} (${hit.orb.toFixed(1)}° orb), a classic marker for visible chapter changes.`,weight:w})
            }
          }

          if(area.houses.includes(profHouse) && natalLord && (t.name==='Jupiter'||t.name==='Saturn')){
            const hit=closestMajorAspect(t.longitude,natalLord.longitude,3.2)
            if(hit){
              const w=.58*Math.max(.2,1-hit.orb/3.2)
              score+=w
              if(w>.2)items.push({label:'Time-lord contact',detail:`${t.name} contacts natal ${lord}, the Lord of the Year, by ${hit.name.toLowerCase()} (${hit.orb.toFixed(1)}° orb).`,weight:w})
            }
          }
        }
        return {score,items}
      ...
      let cycleBonus=0
      const cycleEvidence:MilestoneIndicator[]=[]
      if(natalJupiter && area.houses.includes(natalJupiter.house)){
        let best:{hit:ReturnType<typeof closestMajorAspect>;month:number}|null=null
        months.forEach((rows,month)=>{
          const t=rows.find(p=>p.name==='Jupiter');if(!t)return
          const hit=closestMajorAspect(t.longitude,natalJupiter.longitude,2.4)
          if(hit&&['Conjunction','Opposition','Trine','Square'].includes(hit.name)&&(!best||hit.orb<(best.hit?.orb??99)))best={hit,month}
        })
        if(best&&best.hit){
          const w=best.hit.name==='Conjunction'?1.25:.72
          cycleBonus+=w
          cycleEvidence.push({label:'Jupiter cycle',detail:`Jupiter reaches a ${best.hit.name.toLowerCase()} to natal Jupiter, renewing a roughly 12-year developmental cycle connected to natal House ${natalJupiter.house}.`,weight:w})
        }
      }
      if(natalSaturn && area.houses.includes(natalSaturn.house)){
        let best:{hit:ReturnType<typeof closestMajorAspect>;month:number}|null=null
        months.forEach((rows,month)=>{
          const t=rows.find(p=>p.name==='Saturn');if(!t)return
          const hit=closestMajorAspect(t.longitude,natalSaturn.longitude,2.4)
          if(hit&&['Conjunction','Opposition','Square'].includes(hit.name)&&(!best||hit.orb<(best.hit?.orb??99)))best={hit,month}
        })
        if(best&&best.hit){
          const w=best.hit.name==='Conjunction'?1.6:1.05
          cycleBonus+=w
          cycleEvidence.push({label:'Saturn cycle',detail:`Saturn reaches a ${best.hit.name.toLowerCase()} to natal Saturn, marking a structural maturation point tied to natal House ${natalSaturn.house}.`,weight:w})
        }
      }

      const maxMonth=Math.max(...monthScores.map(m=>m.score))
      const peakMonth=Math.max(0,monthScores.findIndex(m=>m.score===maxMonth))
      const avgMonth=monthScores.reduce((sum,m)=>sum+m.score,0)/12
      const score=annual+maxMonth*.82+avgMonth*.32+cycleBonus
      if(score<3.55)continue

      const peakDate=monthDates[peakMonth] || monthDates[0]
      const actualPeakYear=peakDate.getUTCFullYear(),actualPeakMonth=peakDate.getUTCMonth()
      const actualAge=ageOnDate(birthDate,peakDate)
      const currentYear=now.getFullYear(),currentMonth=now.getMonth()
      const status:MajorChapter['status']=actualPeakYear<currentYear||(actualPeakYear===currentYear&&actualPeakMonth<currentMonth)?'Past':actualPeakYear===currentYear&&Math.abs(actualPeakMonth-currentMonth)<=1?'Current':'Future'
      const indicators=dedupeIndicators([...annualEvidence,...(monthScores[peakMonth]?.items||[]),...cycleEvidence]).slice(0,5)
      candidates.push({
        id:`${area.key}-${actualPeakYear}-${actualPeakMonth}`,area:area.name,icon:area.icon,houses:area.houses,
        peakYear:actualPeakYear,peakMonth:actualPeakMonth,age:actualAge,window:monthWindow(actualPeakYear,actualPeakMonth),score,
        strength:milestoneStrength(score),status,manifestations:area.manifestations,indicators
      })
    }
  }

  const choose=(pool:MajorChapter[],limit:number)=>{
    const picked:MajorChapter[]=[]
    for(const c of [...pool].sort((a,b)=>b.score-a.score)){
      if(picked.some(p=>p.area===c.area&&Math.abs(p.peakYear-c.peakYear)<=2))continue
      if(picked.filter(p=>p.peakYear===c.peakYear).length>=2)continue
      picked.push(c)
      if(picked.length>=limit)break
    }
    return picked
  }
  const past=choose(candidates.filter(c=>c.status==='Past'),4)
  const future=choose(candidates.filter(c=>c.status!=='Past'),7)
  return [...past,...future].sort((a,b)=>a.peakYear-b.peakYear||a.peakMonth-b.peakMonth)
}
...
function App(){
  ...
  const [chapterScope,setChapterScope]=useState<ChapterScope>('Future')
  ...
  const chapters=useMemo(()=>{
    if(!chart||!swe)return []
    return majorLifeChapters(swe,chart,date)
  },[chart,swe,date])

  const visibleChapters=useMemo(()=>chapters.filter(c=>chapterScope==='All'||(chapterScope==='Past'?c.status==='Past':c.status!=='Past')),[chapters,chapterScope])
  ...
  return <main className="app">
      ...
      {chapters.length>0&&<section className="majorChapters">
        ...
        <div className="chapterTimeline">
          {visibleChapters.map((chapter,i)=><details className={`chapterCard ${chapter.status.toLowerCase()} strength-${chapter.strength.toLowerCase().replace(' ','-')}`} key={chapter.id} open={i===0&&chapterScope!=='Past'}>
            <summary>
              <div className="chapterRail"><span className="chapterDot"></span><small>AGE {chapter.age}</small><strong>{chapter.peakYear}</strong><em>{chapter.status}</em></div>
          ...
          {visibleChapters.length===0&&<div className="chapterEmpty">No chapter in this filter clears the major-activation threshold. Try <button type="button" onClick={()=>setChapterScope('All')}>All</button>.</div>}
        </div>
    ...
    <section className="credits">
      ...
      <div className="creditGrid">
        ...
        <article><h3>Prediction method</h3><p>Current forecasts combine the natal Whole Sign chart, annual profection, Lord of the Year, traditional planetary transits, natal contacts, applying/separating motion, and topical house mapping. The Major Chapters engine separately scans annual profections, Jupiter/Saturn developmental cycles, house-ruler contacts, and ASC/MC-axis activation for multi-indicator windows. These are symbolic timing judgments rather than guaranteed events.</p></article>
