...
function ChartWheel({chart,transits,transitDate,onSelect}:{chart:ChartData;transits:PlanetRow[];transitDate:string;onSelect:(name:string)=>void}){
  ...
  const aspectR=178
  const natalBaseR=230
  const natalGuideR=262
  const transitBaseR=304
  const transitGuideR=338
  const signInner=366
  const outer=395
  ...
  // Spread bodies that are close in longitude onto adjacent micro-radii.
  const spreadRadii=(rows:PlanetRow[],base:number)=>{
    const sorted=[...rows].sort((a,b)=>a.longitude-b.longitude)
    const out:Record<string,number>={}
    let cluster:PlanetRow[]=[]
    const flush=()=>{
      if(!cluster.length)return
      const offsets=cluster.map((_,i)=>(i-(cluster.length-1)/2)*16)
      cluster.forEach((p,i)=>out[p.name]=base+offsets[i])
      cluster=[]
    }
    sorted.forEach((p,i)=>{
      if(!cluster.length){cluster=[p];return}
      const prev=cluster[cluster.length-1]
      if(angularDistance(prev.longitude,p.longitude)<5.5) cluster.push(p)
      else {flush();cluster=[p]}
      if(i===sorted.length-1) flush()
    })
    if(cluster.length)flush()
    return out
  }

  const natalRadii=spreadRadii(chart.planets,natalBaseR)
  const transitRadii=spreadRadii(transits,transitBaseR)
  ...
  const [ascLX,ascLY]=xy(chart.ascendant,outer+25)
  const [mcX,mcY]=xy(chart.mc,outer)
  const [mcLX,mcLY]=xy(chart.mc,outer+25)
  ...
  return <div className="chartWheelWrap">
    <svg className="chartWheel" viewBox={`0 0 ${size} ${size}`} role="img" aria-label={`Concentric natal and transit chart for ${transitDate}`}>
      ...
      <circle cx={c} cy={c} r={natalBaseR-35} className="natalBoundary inner"/>
      <circle cx={c} cy={c} r={transitGuideR} className="transitBoundary"/>
      <circle cx={c} cy={c} r={transitBaseR-34} className="transitBoundary inner"/>
      ...
      {chart.planets.map(p=>{
        const r=natalRadii[p.name]??natalBaseR
        const [x,y]=xy(p.longitude,r)
        const [dx,dy]=xy(p.longitude,r+29)
      ...
      {transits.map(p=>{
        const r=transitRadii[p.name]??transitBaseR
        const [x,y]=xy(p.longitude,r)
        const [dx,dy]=xy(p.longitude,r+27)
