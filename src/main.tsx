  ...
  for(let age=startAge;age<=endAge;age++){
    ...
    for(const area of MILESTONE_AREAS){
      ...
      if(natalJupiter && area.houses.includes(natalJupiter.house)){
        let best:{hit:NonNullable<ReturnType<typeof closestMajorAspect>>;month:number}|null=null
        for(let month=0;month<months.length;month++){
          const rows=months[month]
          const t=rows.find(p=>p.name==='Jupiter');if(!t)continue
          const hit=closestMajorAspect(t.longitude,natalJupiter.longitude,2.4)
          if(hit&&['Conjunction','Opposition','Trine','Square'].includes(hit.name)&&(!best||hit.orb<best.hit.orb))best={hit,month}
        }
        if(best){
      ...
      if(natalSaturn && area.houses.includes(natalSaturn.house)){
        ...
        for(let month=0;month<months.length;month++){
          const rows=months[month]
          const t=rows.find(p=>p.name==='Saturn');if(!t)continue
          const hit=closestMajorAspect(t.longitude,natalSaturn.longitude,2.4)
          if(hit&&['Conjunction','Opposition','Square'].includes(hit.name)&&(!best||hit.orb<best.hit.orb))best={hit,month}
        }
        if(best){
