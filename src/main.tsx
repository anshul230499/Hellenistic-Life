...
function deityAsset(tradition:'hindu'|'greek'|'roman',slug:string){
  return new URL(`./assets/deities/${tradition}/${slug}.png`, import.meta.url).href
}
...
const ASPECT_MEANINGS:Record<string,string>={
  Conjunction:'Two planets occupy the same region of the zodiac. Their significations are joined and act together.',
  Sextile:'A 60° relationship. Traditionally a mild, cooperative configuration associated with opportunities and workable exchange.',
  Square:'A 90° relationship. A dynamic, tense configuration that tends to require action, effort, adjustment, or confrontation.',
  Trine:'A 120° relationship. A flowing configuration traditionally associated with support, continuity, and easier cooperation.',
  Opposition:'A 180° relationship. Planets face one another across the zodiac, often describing polarization, encounter, culmination, or the need to balance two sides.'
}

function ChartWheel({chart,onSelect}:{chart:ChartData,onSelect:(name:string)=>void}){
  const size=640,c=size/2,outer=292,inner=195,planetR=238
  const xy=(deg:number,r:number)=>{const a=(deg-90)*Math.PI/180;return [c+r*Math.cos(a),c+r*Math.sin(a)] as const}
  const planetByName=Object.fromEntries(chart.planets.map(p=>[p.name,p]))
  return <div className="chartWheelWrap"><svg className="chartWheel" viewBox={`0 0 ${size} ${size}`} role="img" aria-label="Natal Whole Sign chart wheel with aspects">
    <circle cx={c} cy={c} r={outer} className="wheelOuter"/><circle cx={c} cy={c} r={inner} className="wheelInner"/>
    {Array.from({length:12},(_,i)=>{const deg=i*30,[x1,y1]=xy(deg,inner),[x2,y2]=xy(deg,outer),[lx,ly]=xy(deg+15,(inner+outer)/2),sign=(chart.ascSignIndex+i)%12;return <g key={i}><line x1={x1} y1={y1} x2={x2} y2={y2} className="houseRay"/><text x={lx} y={ly-5} textAnchor="middle" className="wheelSign">{SIGNS[sign][1]}</text><text x={lx} y={ly+14} textAnchor="middle" className="wheelHouse">{i+1}</text></g>})}
    {chart.aspects.map((a,i)=>{const p1=planetByName[a.a],p2=planetByName[a.b];if(!p1||!p2)return null;const [x1,y1]=xy(p1.longitude,inner-12),[x2,y2]=xy(p2.longitude,inner-12);return <line key={i} x1={x1} y1={y1} x2={x2} y2={y2} className={`aspectLine aspect-${a.aspect.toLowerCase()}`}/>})}
    {chart.planets.map(p=>{const [x,y]=xy(p.longitude,planetR);return <g key={p.name} className="wheelPlanet" onClick={()=>onSelect(p.name)} role="button" tabIndex={0} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();onSelect(p.name)}}}><circle cx={x} cy={y} r="18"/><text x={x} y={y+6} textAnchor="middle">{p.glyph}</text></g>})}
    <text x={c} y={c-12} textAnchor="middle" className="wheelCenterTitle">{chart.ascSign} Rising</text><text x={c} y={c+12} textAnchor="middle" className="wheelCenterSub">Whole Sign</text>
  </svg></div>
}
  ...
  return <main className="app">
    ...
    <>
      ...
      <section className="wheelSection">
        <div className="sectionHead"><div><p className="eyebrow">THE NATIVITY</p><h2>Whole Sign wheel & aspects</h2>
        <p className="note">Planets are plotted by tropical longitude. The twelve outer sectors follow Whole Sign houses from the Ascendant sign; lines across the center show the five traditional Ptolemaic aspects.</p></div><span className="pill">{chart.ascSign} rising</span></div>
        <div className="wheelLayout"><ChartWheel chart={chart} onSelect={setSelected}/>
          <div className="aspectLegend"><p className="eyebrow">ASPECTS · WHAT THEY MEAN</p>
        ...
        <div className="wheelAspectList">{[...chart.aspects].sort((a,b)=>a.exactness-b.exactness).slice(0,10).map((a,i)=><button type="button" key={i} onClick={()=>setSelected(a.a)}><span>{a.a} — {a.b}</span><strong>{a.aspect}</strong><small>{a.separation.toFixed(2)}° separation · {a.wholeSign?'whole-sign configured':'degree-orb'}</small></button>)}</div>
      </section>
