...
function ChartWheel({chart,transits,transitDate,onSelect}:{chart:ChartData;transits:PlanetRow[];transitDate:string;onSelect:(name:string)=>void}){
  const size=820,c=size/2
  const aspectR=166
  const natalR=218
  const natalGuideR=242
  const transitR=284
  const transitGuideR=309
  const signInner=337
  const outer=365

  const ascStart=chart.ascSignIndex*30

  // Whole Sign House I is centered at 9 o'clock.
  // The horizontal mirror makes the zodiac progress upward from the Ascendant:
  // for Sagittarius rising, Capricorn sits above Sagittarius where Scorpio sat before.
  ...
  const [ascLX,ascLY]=xy(chart.ascendant,outer+24)
  const [mcX,mcY]=xy(chart.mc,outer)
  const [mcLX,mcLY]=xy(chart.mc,outer+24)
  ...
  return <div className="chartWheelWrap">
    <svg className="chartWheel" viewBox={`0 0 ${size} ${size}`} role="img" aria-label={`Concentric natal and transit chart for ${transitDate}`}>
      ...
      {/* 30° Whole Sign sectors. Every boundary extends from the center to the rim. */}
      ...
      <text x={ascLX} y={ascLY} textAnchor="middle" dominantBaseline="middle" className="angleLabel ascAngleLabel">ASC</text>
      <line x1={c} y1={c} x2={mcX} y2={mcY} className="angleLine mcAngle"/>
      <text x={mcLX} y={mcLY} textAnchor="middle" dominantBaseline="middle" className="angleLabel mcAngleLabel">MC</text>

      {/* Natal aspect geometry occupies the clear center field. */}
      ...
      {/* Ring labels are kept outside the aspect field. */}
