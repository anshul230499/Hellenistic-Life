...
function transitEvidenceForDate(swe:SwissEphemeris,chart:ChartData,birthDate:string,date:Date){
  ...
  return transits.map(t=>{
    ...
    const relevance=t.house===profHouse?`Transiting the annually activated House ${profHouse}.`:
      natalLord && best?.target===natalLord?`Contacting ${lord}, the Lord of the Year.`:
...
const PLANET_GUIDES:Record<string,{
  ...
  Sun:{
    Hindu:{name:'Surya',theme:'clarity, vitality, rightful visibility',image:'https://upload.wikimedia.org/wikipedia/commons/1/17/Surya_deva.jpg'},
    Greek:{name:'Helios',theme:'illumination, witness, conscious direction',image:'https://upload.wikimedia.org/wikipedia/commons/5/5b/Helios_relief.jpg'},
    Roman:{name:'Sol',theme:'radiance, authority, steadiness of purpose',image:'https://upload.wikimedia.org/wikipedia/commons/8/88/Sol_Invictus_Roman.jpg'}
  },
  Moon:{
    Hindu:{name:'Chandra',theme:'mind, feeling, receptivity, rhythm',image:'https://upload.wikimedia.org/wikipedia/commons/5/5c/Chandra_deva.jpg'},
    Greek:{name:'Selene',theme:'reflection, cycles, inner response',image:'https://upload.wikimedia.org/wikipedia/commons/6/68/Selene_Louvre.jpg'},
    Roman:{name:'Luna',theme:'rhythm, memory, changing conditions',image:'https://upload.wikimedia.org/wikipedia/commons/7/7a/Luna_Roman_goddess.jpg'}
  },
  Mercury:{
    Hindu:{name:'Budha',theme:'intelligence, speech, discernment, exchange',image:'https://upload.wikimedia.org/wikipedia/commons/thumb/4/40/Budha_graha.JPG/1280px-Budha_graha.JPG'},
    Greek:{name:'Hermes',theme:'messages, crossings, negotiation, travel',image:'https://4.bp.blogspot.com/-F48GJEJRYq4/TXjpyAEFdRI/AAAAAAAAAB0/t6GlxCG-yQ8/w1200-h630-p-k-no-nu/Hermes3.jpg'},
    Roman:{name:'Mercury',theme:'communication, commerce, exchange, movement',image:'https://content.ngv.vic.gov.au/retrieve.php?size=1280&type=image&vernonID=36464'}
  },
  Venus:{
    Hindu:{name:'Shukra',theme:'harmony, attraction, pleasure, reconciliation',image:'https://upload.wikimedia.org/wikipedia/commons/8/88/Shukra_graha.jpg'},
    Greek:{name:'Aphrodite',theme:'beauty, affection, attraction, accord',image:'https://upload.wikimedia.org/wikipedia/commons/0/0e/Aphrodite_Braschi_Glyptothek_Munich_258.jpg'},
    Roman:{name:'Venus',theme:'relationship, grace, pleasure, social harmony',image:'https://upload.wikimedia.org/wikipedia/commons/4/48/Venus_de_Milo_Louvre_Ma399_n4.jpg'}
  },
  Mars:{
    Hindu:{name:'Mangala',theme:'courage, discipline, force, decisive action',image:'https://upload.wikimedia.org/wikipedia/commons/9/97/Mangala_graha.JPG'},
    Greek:{name:'Ares',theme:'assertion, conflict, courage, force',image:'https://upload.wikimedia.org/wikipedia/commons/6/6c/Ares_Borghese_Louvre_Ma866.jpg'},
    Roman:{name:'Mars',theme:'discipline, defense, command, purposeful action',image:'https://upload.wikimedia.org/wikipedia/commons/6/6f/Mars_Ultor_Capitoline_Museums.jpg'}
  },
  Jupiter:{
    Hindu:{name:'Brihaspati',theme:'wisdom, counsel, teachers, right judgment',image:'https://upload.wikimedia.org/wikipedia/commons/7/70/Brihaspati.jpg'},
    Greek:{name:'Zeus',theme:'authority, order, protection, expansive judgment',image:'https://upload.wikimedia.org/wikipedia/commons/c/c8/Jupiter_Smyrna_Louvre_Ma13.jpg'},
    Roman:{name:'Jupiter',theme:'law, blessing, authority, enlargement',image:'https://upload.wikimedia.org/wikipedia/commons/c/c8/Jupiter_Smyrna_Louvre_Ma13.jpg'}
  },
  Saturn:{
    Hindu:{name:'Shani',theme:'endurance, karma, limits, patience, responsibility',image:'https://upload.wikimedia.org/wikipedia/commons/5/56/Shani_deva.jpg'},
    Greek:{name:'Kronos',theme:'time, limits, consequence, maturation',image:'https://upload.wikimedia.org/wikipedia/commons/a/a4/Kronos_Altemps_Inv8590.jpg'},
    Roman:{name:'Saturn',theme:'time, restraint, agriculture, mature order',image:'https://upload.wikimedia.org/wikipedia/commons/a/a4/Kronos_Altemps_Inv8590.jpg'}
  }
}
    ...
    <>
      ...
      {guidingTransit && PLANET_GUIDES[guidingTransit.transit] && <section className="devotionalToday">
        ...
        <div className="deityFeature">
          <figure>
            <img src={PLANET_GUIDES[guidingTransit.transit][devotionalTradition].image} alt={`${PLANET_GUIDES[guidingTransit.transit][devotionalTradition].name}, ${devotionalTradition} devotional or classical representation`}/>
          </figure>
          <div>
      ...
      {chart && <section className="natalDevotion">
        ...
        <div className="natalGuideGrid">{Array.from({length:12},(_,i)=>{
          ...
          return <article key={i}>
            <img src={d.image} alt={`${d.name}, guide for House ${i+1}`}/>
