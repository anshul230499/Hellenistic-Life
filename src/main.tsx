...
type OlympusComic={title:string;weather:string;beats:ComicBeat[]}
...
function comicName(planet:string,tradition:DevotionalTradition){
  const v=COMIC_VOICES[planet]; return v ? v[tradition==='Roman'?'Roman':'Greek'] : planet
}
function dailyComic(transits:PlanetRow[],date:string,tradition:DevotionalTradition):OlympusComic{
  ...
  if(!hit){
    const a=transits[seed%transits.length],b=transits[(seed+3)%transits.length],A=comicName(a.name,tradition),B=comicName(b.name,tradition)
    return{title:'A quiet day in the divine court',weather:`${A} and ${B} preside over a relatively quiet sky.`,beats:[
      {speaker:A,line:'Nothing dramatic on the docket. Suspicious.'},
      {speaker:B,line:'Try enjoying the silence before someone aspects someone.'},
      {speaker:A,line:'Fine. But I am keeping my sandals on.'}]}
  }
  const A=comicName(hit.a.name,tradition),B=comicName(hit.b.name,tradition)
  const scripts:Record<string,ComicBeat[][]>={
    conjunction:[[{speaker:A,line:'Why are you in my chariot?'},{speaker:B,line:'Apparently the cosmos double-booked us.'},{speaker:'Olympus',line:'HR has described the situation as “energetic.”'}]],
    sextile:[[{speaker:A,line:'I found a shortcut.'},{speaker:B,line:'Is it legal?'},{speaker:'Olympus',line:'Hermes has left the group chat.'}]],
    square:[[{speaker:A,line:'TODAY, NOTHING STANDS IN MY WAY.'},{speaker:B,line:'Wonderful. Please complete Form 27-B.'},{speaker:'Olympus',line:'The hero’s journey now includes administrative processing.'}]],
    trine:[[{speaker:A,line:'Everything is going suspiciously well.'},{speaker:B,line:'Do not say that out loud.'},{speaker:'Olympus',line:'Hubris has enabled notifications.'}]],
    opposition:[[{speaker:A,line:'You are clearly the problem.'},{speaker:B,line:'I was literally about to say that.'},{speaker:'Olympus',line:'Two gods discover perspective. Historians are stunned.'}]]
  }
  return{title:`${hit.kind[0].toUpperCase()+hit.kind.slice(1)} in the divine court`,weather:`Inspired by the selected day's ${hit.a.name}–${hit.b.name} ${hit.kind} · ${hit.orb.toFixed(1)}° orb`,beats:scripts[hit.kind][0]}
}

function ComicDeity({name,panel}:{name:string;panel:number}){
  const n=name.toLowerCase()
  const kind=n.includes('zeus')||n.includes('jupiter')?'zeus':
    n.includes('hermes')||n.includes('mercury')?'hermes':
    n.includes('aphrodite')||n.includes('venus')?'venus':
    n.includes('ares')||n==='mars'?'mars':
    n.includes('kronos')||n.includes('saturn')?'saturn':
    n.includes('helios')||n==='sol'?'sun':
    n.includes('selene')||n.includes('luna')?'moon':'chorus'
  return <div className={`toon toon-${kind} toon-p${panel}`} aria-hidden="true">
    <div className="toonHair"></div><div className="toonHead"><i className="eye e1"></i><i className="eye e2"></i><i className="brow b1"></i><i className="brow b2"></i><i className="mouth"></i></div>
    <div className="toonBody"></div><div className="toonArm arm1"></div><div className="toonArm arm2"></div>
    {kind==='zeus'&&<><b className="beard"></b><b className="bolt">ϟ</b></>}
    {kind==='hermes'&&<><b className="wing w1">◁</b><b className="wing w2">▷</b><b className="staff">☤</b></>}
    {kind==='venus'&&<><b className="rose">✿</b><b className="shell">◡</b></>}
    {kind==='mars'&&<><b className="helmet"></b><b className="spear">↟</b></>}
    {kind==='saturn'&&<><b className="clock">◷</b><b className="scroll">FORM<br/>27-B</b></>}
    {kind==='sun'&&<b className="halo">☉</b>}
    {kind==='moon'&&<b className="crescent">☾</b>}
  </div>
}
...
function App(){
  ...
  const olympusComic=chart&&wheelTransits.length?dailyComic(wheelTransits,wheelDate,devotionalTradition):null
  ...
  return <main className="app">
    ...
    <section className="olympusComic" aria-labelledby="olympus-comic-title">
      <div className="comicHeading">
        <div><p className="eyebrow">A CELESTIAL INTERLUDE</p><h2 id="olympus-comic-title">Meanwhile, on Olympus…</h2></div>
        {olympusComic&&<span>{wheelDate}</span>}
      </div>
      {olympusComic ? <>
        <p className="comicWeather">{olympusComic.weather}</p>
        <div className="comicStrip">
          {olympusComic.beats.map((beat,i)=><article className={`comicPanel panel${i+1}`} key={`${beat.speaker}-${i}`}>
            <div className="comicSun" aria-hidden="true">☉</div>
            <div className="temple" aria-hidden="true"><i></i><i></i><i></i><i></i></div>
            <ComicDeity name={beat.speaker} panel={i}/>
            <div className="speech"><strong>{beat.speaker}</strong><p>{beat.line}</p></div>
            {i===2&&<div className="punchTag">CELESTIAL CONSEQUENCES</div>}
            <small>OLYMPUS · {i+1}</small>
          </article>)}
        </div>
        <div className="comicCaption"><strong>{olympusComic.title}</strong><span>Generated from the selected day's planetary weather. Change the chart-wheel date to reveal another episode.</span></div>
      </> : <p className="comicEmpty">Calculate your chart to open today's scene on Olympus.</p>}
    </section>
