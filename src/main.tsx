...
function comicPortrait(planet:string,tradition:ComicTradition){
  return PLANET_GUIDES[planet]?.[tradition]?.image || DEITY_IMAGES.classical.jupiter
}
...
function App(){
  ...
  return <main className="app">
    ...
    <section className="olympusComic" aria-labelledby="olympus-comic-title">
      <div className="comicMasthead">
        <div className="comicTitleRule"><span>→</span><h2 id="olympus-comic-title">MEANWHILE, ON OLYMPUS…</h2><span>🏛</span></div>
        <p>TODAY'S COSMIC WEATHER IN THREE ACTS</p>
        <div className="comicDateBlock"><strong>{wheelDate}</strong><small>Daily episode</small></div>
      </div>
      <div className="comicControls" role="group" aria-label="Comic mythology tradition">
        <span>Cast</span>
        <button className={comicTradition==='Greek'?'active':''} onClick={()=>setComicTradition('Greek')}>Greek</button>
        <button className={comicTradition==='Roman'?'active':''} onClick={()=>setComicTradition('Roman')}>Roman</button>
      </div>
      {olympusComic ? <>
        <h3 className="comicStory">TODAY'S MAIN STORY: <em>{olympusComic.title}</em></h3>
        <div className="comicStrip realisticStrip">
          {olympusComic.beats.map((beat,i)=>{
            const planet=olympusComic.planets[i]
            const guide=PLANET_GUIDES[planet]?.[comicTradition]
            return <article className={`comicPanel realisticPanel panel${i+1} deity-${planet.toLowerCase()}`} key={`${beat.speaker}-${i}`}>
              <div className="panelDialogue">
                <span className="panelNumber">{i+1}</span>
                <div className="speech realisticSpeech"><p>{beat.line}</p></div>
              </div>
              <div className="panelArtwork">
                <div className="olympusSky" aria-hidden="true"><i></i><i></i><i></i></div>
                <div className="olympusTemple" aria-hidden="true"><i></i><i></i><i></i><i></i></div>
                <img src={comicPortrait(planet,comicTradition)} alt="" className="comicPortrait"/>
                <div className="comicAtmosphere" aria-hidden="true"></div>
                <div className="deityMotif" aria-hidden="true"></div>
                <div className="deityPlaque"><strong>{beat.speaker}</strong><small>{guide?.theme}</small></div>
              </div>
            </article>
          })}
        </div>
        <div className="comicExplainerGrid">
          <article className="flairBox"><p className="comicKicker">✦ THE FLAIR</p><p>{olympusComic.flair}</p><div className="keywordLine"><strong>Keywords</strong><span>{olympusComic.keywords.join(' · ')}</span></div></article>
          <article className="definitionBox"><p className="comicKicker">▣ THE DEFINITION</p><p>{olympusComic.definition}</p><blockquote>Read the symbolism; don't force the event.</blockquote></article>
          <article className="exampleBox"><p className="comicKicker">🏛 EXAMPLE IN LIFE</p><p>{olympusComic.example}</p></article>
          <aside className="todayGods"><p className="comicKicker">TODAY'S GODS</p>{olympusComic.planets.map((planet,i)=>{const g=PLANET_GUIDES[planet]?.[comicTradition];return <div key={`${planet}-${i}`}><img src={comicPortrait(planet,comicTradition)} alt=""/><span><strong>{g?.name||planet}</strong><small>{planet}</small></span></div>})}</aside>
        </div>
        <div className="comicFoot"><strong>🏛 Hellenistic Life</strong><span>Ancient wisdom. Timeless guidance.</span><small>Episode generated from the selected day's planetary geometry.</small></div>
