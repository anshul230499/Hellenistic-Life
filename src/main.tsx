...
import './styles.css'
import olympusDailyArt from './assets/deities/olympus-daily-venus-jupiter.png'
    ...
    <section className="credits">
      ...
      <details className="creditDetails"><summary>Detailed deity image credits</summary>
        <div className="imageCreditList">
          ...
          <p><strong>Kronos / Saturn:</strong> Head identified as Kronos/Saturn, Vatican Museums; Wikimedia Commons source and license noted on the <section className="olympusComic olympusIllustrated" aria-labelledby="olympus-comic-title">
      <div className="comicHeading illustratedHeading">
        <div>
          <p className="eyebrow">A CELESTIAL INTERLUDE</p>
          <h2 id="olympus-comic-title">Meanwhile, on Olympus…</h2>
          <p className="comicSubhead">Today’s cosmic weather in three acts</p>
        </div>
      ...
      {olympusComic ? <>
        <div className="olympusArtworkFrame">
          <img src={olympusDailyArt} alt="Original Hellenistic Life illustration: Aphrodite, Jupiter and Hermes in a three-panel scene on Olympus." />
          <div className="artworkBadge">ORIGINAL HELLENISTIC LIFE ILLUSTRATION</div>
        </div>

        <div className="liveStoryBar">
          <span>TODAY’S LIVE STORY</span>
          <strong>{olympusComic.title}</strong>
          <em>{olympusComic.weather}</em>
        </div>

        <div className="comicLoreGrid">
          <article className="loreCard flairCard">
            <p className="loreKicker">✦ THE FLAIR</p>
            <h3>{olympusComic.flair}</h3>
            <div className="keywordRow">{olympusComic.keywords.map(k=><span key={k}>{k}</span>)}</div>
          </article>
          <article className="loreCard definitionCard">
            <p className="loreKicker">▣ THE DEFINITION</p>
            <p>{olympusComic.definition}</p>
          </article>
          <article className="loreCard exampleCard">
            <p className="loreKicker">⌂ EXAMPLE IN LIFE</p>
            <p>{olympusComic.example}</p>
          </article>
          <article className="loreCard godsCard">
            <p className="loreKicker">♜ TODAY’S GODS</p>
            <div className="godMiniList">
              {olympusComic.beats.slice(0,3).map((beat,i)=><div key={`${beat.speaker}-${i}`}><b>{beat.speaker}</b><span>{i===0?'opens the scene':i===1?'answers the moment':'delivers the punchline'}</span></div>)}
            </div>
          </article>
        </div>

        <p className="comicDynamicNote">The illustrated episode is original artwork. The live story, aspect definition, flair, keywords and life example continue to update from the selected chart-wheel date.</p>
      </> : <p className="comicEmpty">Calculate your chart to open today’s scene on Olympus.</p>}
    </section>

    e your chart to open today's scene on Olympus.</p>}
    </section>
