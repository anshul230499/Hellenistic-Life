...
function ChartWheel({chart,transits,transitDate,onSelect}:{chart:ChartData;transits:PlanetRow[];transitDate:string;onSelect:(name:string)=>void}){
  const size=760,c=size/2
  const outer=344,signInner=278,transitR=252,natalR=214,aspectR=174
  const ascStart=chart.ascSignIndex*30
  const visualDeg=(lon:number)=>165+norm(lon-ascStart) // House I spans 165°–195°, centered on the left/9 o’clock.
  const xy=(lon:number,r:number)=>{const a=visualDeg(lon)*Math.PI/180;return [c+r*Math.cos(a),c+r*Math.sin(a)] as const}
  const housePoint=(house:number,r:number,offset=15)=>{const a=(165+(house-1)*30+offset)*Math.PI/180;return [c+r*Math.cos(a),c+r*Math.sin(a)] as const}
  const planetByName=Object.fromEntries(chart.planets.map(p=>[p.name,p]))
  const tAspects=transitNatalAspects(transits,chart.planets,3).slice(0,16)
  const aspectClass=(a:string)=>`aspectLine aspect-${a.toLowerCase()}`

  return <div className="chartWheelWrap">
    <svg className="chartWheel" viewBox={`0 0 ${size} ${size}`} role="img" aria-label={`Natal chart with transits for ${transitDate}`}>
      <circle cx={c} cy={c} r={outer} className="wheelOuter"/>
      <circle cx={c} cy={c} r={signInner} className="wheelSignRing"/>
      <circle cx={c} cy={c} r={transitR+25} className="wheelTransitRing"/>
      <circle cx={c} cy={c} r={natalR-25} className="wheelInner"/>
      {Array.from({length:12},(_,i)=>{
        const a=(165+i*30)*Math.PI/180
        const x1=c+signInner*Math.cos(a),y1=c+signInner*Math.sin(a),x2=c+outer*Math.cos(a),y2=c+outer*Math.sin(a)
        const [lx,ly]=housePoint(i+1,(signInner+outer)/2)
        const sign=(chart.ascSignIndex+i)%12
        return <g key={i}><line x1={x1} y1={y1} x2={x2} y2={y2} className="houseRay"/><text x={lx} y={ly-7} textAnchor="middle" className="wheelSign">{SIGNS[sign][1]}</text><text x={lx} y={ly+14} textAnchor="middle" className="wheelHouse">{i+1}</text></g>
      })}
      <text x="22" y={c-13} className="ascLabel">ASC</text><line x1="60" y1={c} x2={c-outer} y2={c} className="ascLine"/>

      {chart.aspects.slice(0,18).map((a,i)=>{const p1=planetByName[a.a],p2=planetByName[a.b];if(!p1||!p2)return null;const [x1,y1]=xy(p1.longitude,aspectR),[x2,y2]=xy(p2.longitude,aspectR);return <line key={`n-${i}`} x1={x1} y1={y1} x2={x2} y2={y2} className={aspectClass(a.aspect)}/>})}
      {tAspects.map((a,i)=>{const [x1,y1]=xy(a.transitLon,aspectR+7),[x2,y2]=xy(a.natalLon,aspectR-7);return <line key={`t-${i}`} x1={x1} y1={y1} x2={x2} y2={y2} className={`${aspectClass(a.aspect)} transitAspect`}/>})}

      {chart.planets.map(p=>{const [x,y]=xy(p.longitude,natalR);return <g key={`n-${p.name}`} className="wheelPlanet natalPlanet" onClick={()=>onSelect(p.name)}><circle cx={x} cy={y} r="18"/><text x={x} y={y+6} textAnchor="middle">{p.glyph}</text></g>})}
      {transits.map(p=>{const [x,y]=xy(p.longitude,transitR);return <g key={`t-${p.name}`} className="wheelPlanet transitPlanet"><circle cx={x} cy={y} r="15"/><text x={x} y={y+5} textAnchor="middle">{p.glyph}</text></g>})}

      <text x={c} y={c-18} textAnchor="middle" className="wheelCenterTitle">Natal + Transits</text>
      <text x={c} y={c+5} textAnchor="middle" className="wheelCenterSub">House I begins at the Ascendant on the left</text>
      <text x={c} y={c+28} textAnchor="middle" className="wheelCenterDate">{transitDate}</text>
    </svg>
    <div className="ringKey"><span><i className="natalDot"></i>Natal</span><span><i className="transitDot"></i>Transit</span></div>
  </div>
}
  ...
  return <main className="app">
    ...
    <>
      ...
      <section className="wheelSection">
        ...
        <p className="note">House I is anchored to the Ascendant at the left. Natal placements sit on the inner gold ring; transiting placements sit on the outer blue ring. Change the date to watch the sky move around the fixed nativity.</p></div></div>
